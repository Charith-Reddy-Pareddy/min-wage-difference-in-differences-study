...
| Exposure vs. COVID severity | Significant negative correlation, both industries | A candidate mechanism for the exposure-gradient confound |
| Controlling for COVID severity in the placebo test | Placebo effect barely shrinks (8% food service, grows for retail) | COVID severity alone doesn't explain the confound — something else is also going on |
...
- **The exposure-gradient placebo test fails**: at a false 2018
  ...
  finds a real candidate mechanism: a state's pre-2021 exposure share
  correlates significantly with its own COVID severity (food service r =
  -0.35, p = 0.018; retail r = -0.54, p = 0.0001). But
  `R/26_covid_controlled_placebo.R` tests it directly by adding
  `covid_severity x post` to the placebo-date model, and the spurious
  placebo β₄ barely moves (8% shrinkage for food service, retail actually
  grows) — COVID severity is a real, correlated factor, not the full
  explanation. The pre-existing exposure-outcome association documented
  above is real regardless of which underlying variable ultimately
  explains it.
...
R/26_covid_controlled_placebo.R # does controlling for it resolve the placebo failure?
```
