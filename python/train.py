...
bagged trees, and a PyTorch feedforward net, all predicting
employment_growth on the same held-out states -- an independent
replication (different ecosystem, different implementations) of
R/27_ml_prediction.R and R/28_neural_network.R's comparison. Not a
causal claim; see those files' headers for why.
...
from src.tree import BaggedTrees
...
def main() -> pd.DataFrame:
    ...
    results = pd.DataFrame(
        {
            "model": ["linear_baseline", "bagged_trees", "neural_network"],
            "rmse": [rmse(y_test, linear_pred), rmse(y_test, bagged_pred), rmse(y_test, nn_pred)],
            "r_squared": [
